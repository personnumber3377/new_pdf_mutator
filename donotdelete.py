    ...
    for obj in pdf.objects:
        ...
        try:
            ...
            if not isinstance(obj, (pikepdf.Stream, pikepdf.Dictionary, pikepdf.Array)):
                dlog("paskaperseeeeee!!!!")
                # dlog("Not the thing: "+str(obj))
                continue
            dlog("Checking critical")
            if not is_critical_object(obj, pdf):
                dlog("Appending...")
                # dlog("Here is the object: "+str(obj))
...
def mutate_pdf_structural(buf: bytes, max_size: int, rng: random.Random) -> bytes:
    ...
    # In-place mutation path
        ...
        dlog("Choosing target object...")
        target = choose_target_object(pdf, rng)
        dlog("After choosing the thing...")
        if target is None:
